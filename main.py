...
from dutch import build as dutch_build
...
def _ingest_inbound(memory: dict) -> None:
    ...
    if not config.DUTCH_RECALL_ENABLED:
        return
    dutch_build.ingest_recall(inbound, fail=_fail)
...
def _run() -> None:
    ...
    dutch_payload, dutch_state = dutch_build.build(skill["skill"], fail=_fail)
    if dutch_payload:
    ...
    if dutch_state:
        dutch_build.persist(dutch_state, fail=_fail)
