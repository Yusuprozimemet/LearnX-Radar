# LearnX-Radar — architecture

The deep dive behind the [README](../README.md): how the daily run works, why the
numbers are what they are, every config flag, every state file. A visual pipeline
walkthrough also lives in [flowchart.md](../flowchart.md).

## Design principles

Four ideas show up everywhere in this codebase:

1. **Correct by design where LLMs fail.** The LLM writes prose; everything that
   must be *exact* — source attribution, URLs, vocabulary, cloze answers — is
   computed deterministically in code or anchored to human-reviewed data.
2. **Numbers chosen by experiment.** Chunk sizes, read budgets and momentum
   windows were swept against the real corpus with committed harnesses in
   [scripts/](../scripts/), not guessed.
3. **Zero backend.** GitHub Actions is the compute, committed JSON is the
   database, GitHub Releases is the CDN, GitHub Pages is the frontend, and
   Telegram deep links are the inbound channel. No servers, no paid tier.
4. **Graceful degradation.** Every optional secret degrades cleanly, every side
   track is guarded so the core lesson always ships, and stage failures are
...
Seven public feeds: GitHub Trending, Hacker News (Who-is-Hiring + front page),
Stack Overflow tag deltas, dev.to, Reddit, and Lobste.rs. Reddit, the HN front
page and Lobste.rs are open-vocabulary feeds, so the radar can surface skills it
was never pre-configured to watch. Per-source weights (`SOURCE_WEIGHTS`) keep
real demand (HN Hiring, Stack Overflow) above community buzz.

### Extraction: map-reduce + deterministic attribution

`EXTRACTION_MAPREDUCE` — the corpus is chunked and skills extracted per chunk
(recall), variants merged (`SKILL_ALIASES`), then each skill's source set is
computed by **scanning the corpus** rather than trusting the LLM to tally — so
the demand weight is exact. Chunk size was chosen by experiment
([scripts/exp_extraction.py](../scripts/exp_extraction.py)): 6k tokens ties 4k
at the candidate cap with fewer LLM calls, while 8k drops recall — so 6k is the
knee.

### Scoring: demand × novelty × momentum

Gap scoring combines source-weighted demand, spaced-repetition novelty (recently
taught skills sink), and a cross-day momentum signal (`MOMENTUM_ENABLED`) that
looks back over `trending_history` and boosts skills sustained and accelerating
across days while damping one-day spikes.

### Grounded briefs

`GROUNDING_ENABLED` — instead of writing from the skill name alone, the brief
reads the actual sources that surfaced the skill (keyless Jina reader) plus
fresh Exa web results (`EXA_API_KEY`), and cites a real `## Sources` list
authored in code — the LLM never writes URLs. The read budget was chosen by
experiment ([scripts/exp_grounding.py](../scripts/exp_grounding.py)). The
grounding helpers in [radar/research/](../radar/research/) are vendored from the
sibling LearnX-Search project.
...
bank ([dutch/wordlist.json](../dutch/wordlist.json)). The LLM only writes
sentences around fixed words and never invents vocabulary — any generated word
that isn't in the bank is dropped, so a bad generation falls back to the
verified gloss rather than a wrong word. Grow the bank with the one-time
generator (reviewed before committing):
